#!/usr/bin/env python3
"""
agent_call — call Claude Code from inside a Python script.   [MKTG 6620]
...
back structured data your next line of code can use. This is that.
    ...
    from agent_call_claude import agent_call
...
WHAT YOU GET FOR FREE
  1. Structured output is requested in the prompt, then CHECKED, and re-asked once if the reply is malformed -- because this tool has no schema flag.
...
Verified against Claude Code CLI on 2026-08-18. STDLIB ONLY, on purpose.
"""
...
def _build(prompt, schema, key):
    """claude has no schema flag. The schema is folded into the prompt by
    `agent_call` below, and the reply is checked and retried instead.

    That is not a workaround bolted on -- it is the course's own lesson
    applied to the tool: you asked for something, so verify you got it."""
    return [EXE, "-p", prompt, "--output-format", "text"]
