#!/usr/bin/env python3
"""
agent_call — call Codex from inside a Python script.   [MKTG 6620]
...
back structured data your next line of code can use. This is that.
    ...
    from agent_call_codex import agent_call
...
WHAT YOU GET FOR FREE
  1. Structured output is forced natively by codex via --output-schema.
...
Verified against codex-cli 0.147.0 on 2026-08-18. STDLIB ONLY, on purpose.
"""
...
def _build(prompt, schema, key):
    """codex takes the schema as a FILE PATH, so we write one first."""
    cmd = [EXE, "exec", "--skip-git-repo-check"]
    if schema is not None:
        CACHE_DIR.mkdir(parents=True, exist_ok=True)
        path = CACHE_DIR / (key + ".schema.json")
